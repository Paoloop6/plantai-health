...
export interface IChatStorage {
  ...
  createMessage(conversationId: number, role: string, content: string, imageUrl?: string | null): Promise<typeof messages.$inferSelect>;
}
...
export const chatStorage: IChatStorage = {
  ...
  async createMessage(conversationId: number, role: string, content: string, imageUrl?: string | null) {
    const [message] = await db.insert(messages).values({ conversationId, role, content, imageUrl: imageUrl || null }).returning();
    return message;
  },
};
